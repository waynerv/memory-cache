...
from memory_cache.utils import generate_token
...
class UserTestCase(BaseTestCase):
    ...
    def test_index_page(self):
        ...
        self.login(email='locked@memorycache.com', password='123')
    ...
    def test_follow(self):
        ...
        self.login(email='unconfirmed@memorycache.com', password='123')
        response = self.client.post(url_for('user.follow', username='admin'), follow_redirects=True)
        data = response.get_data(as_text=True)
    ...
    def test_change_email(self):
        user = User.query.get(2)
        self.assertEqual(user.email, 'normal@memorycache.com')
        token = generate_token(user=user, operation=Operations.CHANGE_EMAIL, new_email='new@memorycache.com')

        self.login()
        response = self.client.get(url_for('user.change_email', token=token), follow_redirects=True)
        data = response.get_data(as_text=True)
        self.assertIn('Email updated.', data)
        self.assertEqual(user.email, 'new@memorycache.com')
    ...
    def test_notification_setting(self):
        ...
        self.logout()
        self.login(email='admin@memorycache.com', password='123')
